...
class AssetLibrary(AbstractComponentUI):
    ...
    def create_ui(self):
        with gr.Tab(t("asset_library_tab")) as asset_library_ui:
            with gr.Column():
                with gr.Accordion(t("asset_add_local_or_youtube"), open=False) as accordion:
                    remote = t("asset_remote")
                    local = t("asset_local")
                    assetFlows = gr.Radio([remote, local], label="", value=remote)
                    with gr.Column(visible=True) as youtubeFlow:
                        asset_name = gr.Textbox(label=t("name_required"))
                        asset_type = gr.Radio([AssetType.BACKGROUND_VIDEO.value, AssetType.BACKGROUND_MUSIC.value], value=AssetType.BACKGROUND_VIDEO.value, label=t("type"))
                        youtube_url = gr.Textbox(label=t("url_youtube"))
                        add_youtube_link = gr.Button(t("add"))
                    with gr.Column(visible=False) as localFileFlow:
                        local_upload_name = gr.Textbox(label=t("name_required"))
                        upload_type = gr.Radio([AssetType.BACKGROUND_VIDEO.value, AssetType.BACKGROUND_MUSIC.value, AssetType.IMAGE.value], value=AssetType.BACKGROUND_VIDEO.value, interactive=True, label=t("type"))
                        ...
                        upload_button = gr.Button(t("add"))
                        upload_type.change(lambda x: (gr.update(visible="video" in x), gr.update(visible=any(asset_type in x for asset_type in ["audio", "music"])), gr.update(visible=x == "image")), [upload_type], [video_upload, audio_upload, image_upload])
                ...
                with gr.Row():
                    ...
                    with gr.Column(scale=2):
                        gr.Markdown(t("preview"))
                        asset_preview_ui = gr.HTML(self.__get_first_preview)
                        delete_button = gr.Button(f"🗑️ {t('delete')}", scale=0, variant="primary")
    ...
    def __add_youtube_asset(self, asset_name, yt_url, type):
        ...
        return gr.DataFrame.update(value=latest_df), gr.update(value=self.__get_asset_embed(latest_df, 0)), gr.update(value=f"🗑️ {t('delete')} {latest_df.iloc[0]['name']}"), gr.update(open=False), gr.update(choices=AssetComponentsUtils.getBackgroundVideoChoices(), interactive=True), gr.update(choices=AssetComponentsUtils.getBackgroundMusicChoices(), interactive=True)

    def __get_first_preview(self):
        return self.__get_asset_embed(AssetDatabase.get_df(), 0)

    def __delete_clicked(self, button_name):
        asset_name = button_name.split(f"🗑️ {t('delete')} ")[-1]
        AssetDatabase.remove_asset(asset_name)
        data = AssetDatabase.get_df()
        if len(data) > 0:
            return gr.update(value=data), gr.update(value=self.__get_asset_embed(data, 0)), gr.update(value=f"🗑️ {t('delete')} {data.iloc[0]['name']}"), gr.update(choices=AssetComponentsUtils.getBackgroundVideoChoices(), interactive=True), gr.update(choices=AssetComponentsUtils.getBackgroundMusicChoices(), interactive=True)
        return gr.Dataframe.update(value=data), gr.update(visible=True), gr.update(value=f"🗑️ {t('delete')}"), gr.update(choices=AssetComponentsUtils.getBackgroundVideoChoices(), interactive=True), gr.update(choices=AssetComponentsUtils.getBackgroundMusicChoices(), interactive=True)

    def __preview_asset(self, data, evt: gr.SelectData):
        html_embed = self.__get_asset_embed(data, evt.index[0])
        return gr.update(value=html_embed), gr.update(value=f"🗑️ {t('delete')} {data.iloc[evt.index[0]]['name']}")
    ...
    def __get_asset_embed(self, data, row):
        ...
        if "youtube.com" in asset_link:
            ...
            if file_ext in ["mp3", "wav", "ogg"]:
                ...
                embed_html = f'<audio controls><source src="{asset_link}" type="{audio_type}">{t("browser_not_support_video")}</audio>'
            elif file_ext in ["mp4", "webm", "ogg", "mov"]:
                video_type = "video/mp4" if file_ext == "mp4" else f"video/{file_ext}"
                embed_html = f'<video width="{embed_width}" height="{embed_height}" style="max-height: 100%;" controls><source src="{asset_link}" type="{video_type}">{t("browser_not_support_video")}</video>'
            elif file_ext in ["jpg", "jpeg", "png", "gif"]:
                embed_html = f'<img src="{asset_link}" width="{embed_width}" height="{embed_height}">'
            else:
                embed_html = t("unsupported_file_type")
        return embed_html
    ...
    def __verify_and_upload_local_asset(self, upload_type, upload_name, video_path, audio_path, image_path):
        ...
        if not os.path.exists(path_dict[upload_type]):
            raise gr.Error(t("file_not_exist"))
