...
class VideoTranslationUI(AbstractComponentUI):
    ...
    def create_ui(self):
        youtube_option = t("youtube_link")
        video_file_option = t("video_file")
        with gr.Row(visible=False) as video_translation_ui:
            with gr.Column():
                videoType = gr.Radio([youtube_option, video_file_option], label=t("input_video"), value=youtube_option, interactive=True)
                video_path = gr.Video(sources="upload", interactive=True, width=533.33, height=300, visible=False)
                yt_link = gr.Textbox(label=t("youtube_link_input"), interactive=True, visible=False)
                videoType.change(lambda x: (gr.update(visible=x == video_file_option), gr.update(visible=x == youtube_option)), [videoType], [video_path, yt_link])
                ...
                with gr.Column(visible=False) as eleven_tts:
                    language_eleven = gr.CheckboxGroup(self.eleven_language_choices, label=t("language"), value="ENGLISH", interactive=True)
                ...
                with gr.Column(visible=True) as edge_tts:
                    language_edge = gr.CheckboxGroup([lang.value.upper() for lang in Language], label=t("language"), value="ENGLISH", interactive=True)
                ...
                useCaptions = gr.Checkbox(label=t("caption_video"), value=False)
                translateButton = gr.Button(t("translate_video"))
            ...
            translateButton.click(self.inspect_create_inputs, inputs=[videoType, video_path, yt_link, tts_engine, language_eleven, language_edge], outputs=[generation_error]).success(self.translate_video, inputs=[videoType, yt_link, video_path, tts_engine, language_eleven, language_edge, useCaptions, voice_eleven], outputs=[output, video_folder, generation_error])
    ...
    def translate_video(self, videoType, yt_link, video_path, tts_engine, language_eleven, language_edge, use_captions: bool, voice_eleven: str, progress=gr.Progress()) -> str:
        ...
        try:
            for i, language in enumerate(languages):
                ...
                content_translation_engine = MultiLanguageTranslationEngine(voiceModule=voice_module, src_url=yt_link if videoType == t("youtube_link") else video_path, target_language=language, use_captions=use_captions)
                num_steps = content_translation_engine.get_total_steps()

                def logger(prog_str):
                    progress(self.progress_counter / num_steps, t("translating_video", index=i + 1, total=len(languages), message=prog_str))

                content_translation_engine.set_logger(logger)

                for step_num, step_info in content_translation_engine.makeContent():
                    progress(self.progress_counter / num_steps, t("translating_video", index=i + 1, total=len(languages), message=step_info))
                    self.progress_counter += 1

                translated_video_path = content_translation_engine.get_video_output_path()
                current_url = self.shortGptUI.share_url + "/" if self.shortGptUI.share else self.shortGptUI.local_url
                file_url_path = f"{current_url}gradio_api/file={translated_video_path}"
                file_name = translated_video_path.split("/")[-1].split("\\")[-1]
                self.embedHTML += GradioComponentsHTML.get_html_video_template(file_url_path, file_name, width=500)
    ...
    def inspect_create_inputs(self, videoType, video_path, yt_link, tts_engine, language_eleven, language_edge):
        ...
        if videoType == t("youtube_link"):
            ...
            if not video_path or not os.path.exists(video_path):
                raise gr.Error(t("error_invalid_video_file"))
            file_ext = os.path.splitext(video_path)[-1].lower()
            if file_ext not in supported_extensions:
                raise gr.Error(t("error_invalid_video_extension", extensions=', '.join(supported_extensions)))
        if tts_engine == AssetComponentsUtils.ELEVEN_TTS and not len(language_eleven) > 0:
            raise gr.Error(t("error_select_target_languages"))
        if tts_engine == AssetComponentsUtils.EDGE_TTS and not len(language_edge) > 0:
            raise gr.Error(t("error_select_target_languages"))
...
def update_progress(progress, progress_counter, num_steps, num_shorts, stop_event):
    ...
    while not stop_event.is_set():
        ...
        progress(progress_counter / (num_steps * num_shorts), t("rendering_progress", current=dynamic))
        time.sleep(0.1)
